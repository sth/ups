...
/* @(#)ci_stm.c	1.14 16 Apr 1994 (UKC) */
char ups_ci_stm_c_rcsid[] = "$Id$";
...
int *p_ncase;
{
	...
	for (csl = cslist; csl != NULL; csl = next) {
		...
		if (expr == NULL) {
			...
			ci_do_integral_promotions(expr);/* ENUM Fix : Dibyendu 9/1/99 */
	...
	if (ncase == 0)
		...
		p_cs = cstab = (case_labeled_stm_t **)alloc(Parse_apool,
