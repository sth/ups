...
/* @(#)ci_init.c	1.17 04 Jun 1995 (UKC) */
char ups_ci_init_c_rcsid[] = "$Id$";
...
initexpr_t **p_excess;
{
	...
	if (!ie->ie_is_list && ie->ie_expr->ex_exprtype == ET_STRING_CONST
		...
		initialiser = NULL;
		if (dim->di_htype == DT_CONSTANT && count > dim->di_high + 1) {
			diagf(ET_ERROR, ie->ie_expr->ex_lexinfo,
		       "%ld character initialiser for %ld character array `%s'",
				      		count, dim->di_high + 1, name);
		}
		else {
			initialiser = NEW(initialiser_t);
			initialiser->in_inittype = IT_STRINGBYTES;
...
bool is_local;
{
	...
	while (expr->ex_exprtype == ET_CAST || expr->ex_exprtype == ET_PROMOTION) {
		...
		/* Dibyendu : 26/12/98 : This can affect the meaning
		 * of the expression - so complain.
		 */

		/*
		* Dibyendu : 28/1/99
		* We should warn if ignoring the cast will change the meaning
		* of the expression. An unconditional warning generates
		* too many spurious warnings - need to find a way of filtering
		* out such warnings - until then, get rid of the warning.
		*if (expr->ex_exprtype != ET_PROMOTION) {
		*	diagf(ET_WARNING, expr->ex_lexinfo,
		*		"Cast ignored in initialisation of %s `%s'", 
		*		what, name);
...
#if WANT_LDBL
		case TY_LONGDOUBLE:
			in->in_floatval = val.cv_long_double;
...
initexpr_t *ie;
{
	...
	if (v->va_class == CL_DECL) {
		diagf(ET_WARNING, v->va_lexinfo,
		      "Variable `%s' declared `extern' but has initialiser",
								v->va_name);
		v->va_class = CL_EXT;
	}
