...
/* @(#)ci_parse.h	1.10 16 Apr 1994 (UKC) */

#define CI_PARSE_H_INCLUDED
...
#if WANT_LL
	OP_CVT_TO_LONG_LONG, OP_CVT_TO_ULONG_LONG, 
#endif
#if WANT_LDBL
	OP_CVT_TO_LONG_DOUBLE,
#endif
...
typedef enum {
	IT_INTVAL,
	IT_FLOATVAL,
	IT_STRINGADDR,		/* For initialising a pointer */
	IT_STRINGBYTES,		/* For initialising an array of bytes */
	IT_VARADDR,
	IT_FUNCADDR,
	IT_EXPR,
	IT_LIST
...
#if WANT_LL
	long_long_t cv_long_long;
	ulong_long_t cv_ulong_long;
#endif
} constval_t;
...
typedef struct declarationst {
	class_t dn_class;
	type_t *dn__basetype;	/* Accessed via get_dn_basetype() */
	unsigned dn_typeflags;
	unsigned dn_dup_typeflags;
...
typedef struct {
	...
	union {
#if WANT_LL
		long_long_t cou_integer_val;
		ulong_long_t cou_unsigned_val;
#else
		long cou_integer_val;
		unsigned long cou_unsigned_val;
#endif
...
#define co_unsigned_val  co_u.cou_unsigned_val
...
#if WANT_LL
		long long exu_integer_constant_val;
#else
...
typedef struct case_labeled_stmst {
	...
	long cs_val;
	expr_t *cs_expr;
	struct statement_s *cs_stm;
} case_labeled_stm_t;

typedef struct compound_stmst {
	block_t *co_block;
	struct statement_s *co_statements;
} compound_stm_t;

typedef struct if_stmst {
	expr_t *is_expr;
	struct statement_s *is_ifpart;
	struct statement_s *is_elsepart;
} if_stm_t;

typedef struct switch_stmst {
	expr_t *ss_expr;
	struct statement_s *ss_stm;
