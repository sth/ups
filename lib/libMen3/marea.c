...
/* @(#)marea.c	1.7 26/4/92 (UKC) */
char Men3_marea_c_rcsid[] = "$Id$";
...
const char *string;
{
	...
	while ((w = wn_wdbyte(ma->msg_wfd,db,(Box_t *)0,WN_FG(ma->msg_wfd),WN_BG(ma->msg_wfd))) > 0) {
#ifdef DEBUG
...
MAREA * ma;
{
	wn_set_area(ma->msg_wfd,ma->msg_x,ma->msg_dba.DBY - Char_height,
			ma->msg_width,Char_height+2,WN_BG(ma->msg_wfd));
...
MAREA * ma;
{
	...
	s[0] = c;
	db->DBSrcString = s;
	while ((w = wn_wdbyte(ma->msg_wfd,db,(Box_t *)0,WN_FG(ma->msg_wfd),WN_BG(ma->msg_wfd))) == 1)
		scroll(ma);
	return(w);
}
...
int n;
{
	...
	while (n--) {
		...
		db->DBMaxByte = 1;
		db->DBSrcString = " ";
		wn_wdbyte(ma->msg_wfd,db,(Box_t *)0,WN_FG(ma->msg_wfd),WN_BG(ma->msg_wfd));
		db->DBX -= Char_width;
	}
}
