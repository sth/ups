...
/* @(#)mopen.c	1.14 17 Apr 1994 (UKC) */
char Men3_mopen_c_rcsid[] = "$Id$";
...
const char * name;
{
	...
	Mget_menu_colors(_menu_[i].om_root->me_cap,
			 &(_menu_[i].om_root->me_fg_colour),
			 &(_menu_[i].om_root->me_bg_colour),
			 &fg_specified, &bg_specified);
	Mpropagate_colors(_menu_[i].om_root,
			 _menu_[i].om_root->me_fg_colour,
			 _menu_[i].om_root->me_bg_colour);
	_menu_[i].om_md = i;
...
char *mname;
{
	...
	if ((nodenum = do_struct_header(fp,mname)) == -1||
	    ...
	    getlongnum(fp, &mnode->me_fg_colour,',')	||
	    getlongnum(fp, &mnode->me_bg_colour,',')	||
	    skipnull(fp,',') /* me_save */		||
	    getnodenum(fp, &mnode->me_topleft,',')	||
	    getnodenum(fp, &mnode->me_botrite,',')	||
	    skipnull(fp,',') /* me_parent */		||
	    skipnull(fp,',') /* me_omen */		||
	    skipnull(fp,',') /* me_toggle */		||
	    skipnull(fp,',') /* me_select_val */	||
	    skipwhite(fp,'}')				||
	    skipwhite(fp,';'))
		return(-1);
	mnode->me_flags = flags;
	mnode->me_save = (long)NULL;
...
int expected;
{
	...
	switch ((ch = skipwhite(fp,ANY_CHAR) ))
	{
	case '0':
	   ch =  skipwhite(fp,ANY_CHAR);
	   break;
	case 'N':
	   ch =  checkword(fp,"ULL") != 0 ? -1 : skipwhite(fp,ANY_CHAR);
	   break;
	}

	/* Allow any number of implicit NULL's or 0's at end of structure */
	if ( ch == '}')
	{
            myungetc(ch,fp);
	    ch = expected;
	}
	return (ch == expected ? 0 : -1);
}
...
char *buf;
{
	...
	if ((ch = skipwhite(fp, ANY_CHAR)) == -1 ||
	    !((unsigned char)ch == ch && isalpha(ch)))
		return(-1);
	while ((unsigned char)ch == ch &&
...
short* p_value;
{
    ...
    int ch = mygetc(fp);
    int j;
    while ((isalpha(ch) || ch == '_') && i < (sizeof(name_buff)-1))
    {
	name_buff[i++] = ch;
	ch = mygetc(fp);
    }
    myungetc(ch, fp);
    if ( i < (sizeof(name_buff)))
    {
	name_buff[i] = 0;
	for ( j = 0; j < (sizeof(values)/sizeof(values[0])); j++)
	{
	    if ( !strcmp(name_buff, values[j].name ) )
	    {
		*p_value = values[j].value;
		return 0;
	    }
	}
    }
    ...
    while(i)
	myungetc(name_buff[--i],fp);
    return -1;
}

static int
getnumber(fp, p_res, base, minus)
FILE *fp;
short *p_res;
int base;
int minus;
{
	...
	if ((unsigned char)ch == ch && islower(ch))
		ch = toupper(ch);
...
int followch;
{
   /* Get a short value, in decimal, hex, octal, as a character, or
   ** as one of the flags recoginized by get_named_value().
   ** Accept expressions envolving such values combined by '+'
   */
	short res;
	int ret = 0;
	int ch, base, minus;
	...
	if (!ret &&  base )
	{
	    ret = getnumber(fp, &res, base, minus);
	}
	if (!ret)
	{
	    ch = skipwhite(fp,ANY_CHAR);
