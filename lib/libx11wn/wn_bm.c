...
/* @(#)wn_bm.c	1.13 4/11/93 (UKC) */
char wn_wn_bm_c_rcsid[] = "$Id$";
...
_wn_define_machine_bitmap_format()
{
	...
	if (wn__Dpy == NULL) {
		/*  The display has not been opened yet, so just use a set
		 *  of reasonable values.  These values are only for
		 *  efficiency anyway, so we will not break anything of they
		 *  are wrong.  If the display is subsequently opened, we
		 *  will be called again to set the correct values.
		 */
		Lineinc_align = 32;
		Display_bit_order = BM_BIT0_RIGHT;
		Display_byte_order = BM_MSB_FIRST;
	}
	else {
		Lineinc_align = BitmapPad(wn__Dpy);
		Display_bit_order = (BitmapBitOrder(wn__Dpy) == LSBFirst) ?
						BM_BIT0_LEFT : BM_BIT0_RIGHT;
		Display_byte_order = (ImageByteOrder(wn__Dpy) == LSBFirst) ?
						BM_LSB_FIRST : BM_MSB_FIRST;
	}
#endif /* X11 */
}
...
int lineinc;
{
	...
	memset((char *)dbuf, '\0', (size_t)(l * lineinc));
	ddata = dbuf;
...
int width, height, nplanes, bit_order, pixel_format;
{
	...
	if (bm == NULL)
		...
		memset((char *)data, '\0', (size_t)nbytes);
		bm->bm_flags |= BM_CAN_FREE_DATA;
	}
	return bm;
}
