...
/* @(#)wn.h	1.23 31/7/94 (UKC) */

#ifndef WN_H_INCLUDED
#define WN_H_INCLUDED

/* We need to know options selected.
 */
 ...
 */
#define wn_make_static_X11bm(_width, _height, _bits) \
	wn_make_static_bm(_width, _height, 1, 0, 0, \
		BM_BIT0_LEFT,BM_LSB_FIRST,BM_XY_PIXELS, \
		(_width+7)/8, (unsigned short*)_bits)
...
/*  Macro to print NULL string.
 */
#define WN_PNULL(a)	((a) ? (a) : "<null>")
...
#define wn_rop_from_mem(bm, sx, sy, width, height, wn, dx, dy, ropfunc) \
	wn_put_image(bm, sx, sy, width, height, wn, dx, dy, ropfunc, \
	WN_FG(wn), WN_BG(wn))
...
#define WN_MY_PIXEL	0x80000000
#define WN_FG(wn)	(0 | WN_MY_PIXEL)
#define WN_BG(wn)	(1 | WN_MY_PIXEL)
#else
#define WN_FG(wn)	wn_fg_pixel(wn)
#define WN_BG(wn)	wn_bg_pixel(wn)
#endif
...
#define F2_KEY		-94	        /* insert UPS_F2_STR   		*/
#define F3_KEY		-93	        /* insert UPS_F3_STR   		*/
#define F4_KEY		-92	        /* insert UPS_F4_STR   		*/
#define F5_KEY		-91	        /* insert UPS_F5_STR   		*/
#define F6_KEY		-90	        /* insert UPS_F6_STR   		*/
#define F7_KEY		-89	        /* insert UPS_F7_STR   		*/
#define F8_KEY		-88	        /* insert UPS_F8_STR   		*/
#define F9_KEY		-87	        /* insert UPS_F9_STR   		*/
#define F10_KEY		-86	        /* insert UPS_F10_STR   	*/
#define F11_KEY		-85	        /* insert UPS_F11_STR   	*/
#define F12_KEY		-84	        /* insert UPS_F12_STR   	*/
 ...
 */
#define WN_CH_LEFT_ARROW	((char)150)
#define WN_CH_RIGHT_ARROW	((char)151)
#define WN_CH_UP_ARROW		((char)152)
#define WN_CH_DOWN_ARROW	((char)153)
#define WN_CH_FUNCKEY(n)	((char)(160 + (n)))
...
/*  Any keyboard modifier ( but not B_MULT_CLICKS
 */
#define B_ANY_MODIFIER	(B_SHIFT_MASK | B_CONTROL_MASK | B_ALT_MASK)
...
/* AMC enhancements */
#define WN_MAXWIDTH 0x7fff
#define wn_text(wn, font, s, x, y, fg_color, bg_color, use_which) \
	wn_xtext(wn, font, s, x, y, WN_MAXWIDTH, R_RPL, fg_color, bg_color, use_which, 0)

#define wn_mono_text(wn, font, s, x, y, ropfunc, use_which) \
	wn_xtext(wn, font, s, x, y, WN_MAXWIDTH, ropfunc, WN_FG(wn), WN_BG(wn), use_which, 1)

#define wn_wd_text(wn, font, s, x, y, width, fg_color, bg_color, use_which) \
	wn_xtext(wn, font, s, x, y, width, R_RPL, fg_color, bg_color, use_which, 0)

/*  Ditto for wn_xwrite
 */
#define wn_write(wn, font, s, len, x, y, fg_color, bg_color, use_which) \
	wn_xwrite(wn, font, s,len, x,y, R_RPL, fg_color, bg_color, use_which, 0)

#define wn_mono_write(wn, font, s, len, x, y, ropfunc, use_which) \
	wn_xwrite(wn, font, s,len, x,y, ropfunc, WN_FG(wn), WN_BG(wn), use_which, 1)
...
#define wn_wd_ttext(wn, s, x, y, width,  fg_color, bg_color) \
	wn_wd_text(wn, (font_t *)NULL, s, x, y, width,  fg_color, bg_color, WN_USE_TOP)  

/*  Simpler interfaces to wn_write
 */
#define wn_twrite(wn, s, len, x, y, fg_color, bg_color) \
	wn_write(wn, (font_t *)NULL, s,len, x,y, fg_color, bg_color, WN_USE_TOP)  

#define wn_bwrite(wn, s, len, x, y, fg_color, bg_color) \
	wn_write(wn, (font_t *)NULL, s,len, x,y, fg_color, bg_color, WN_USE_BASELINE)

/* AMC enhancements */
#define wn_xputs(wn, font, s, x, y, func, use_which) \
	wn_xtext(wn, font, s, x, y, WN_MAXWIDTH, func, WN_FG(wn), WN_BG(wn), use_which, 0)
#define wn_bputs(wn,s,x,y)	wn_btext(wn, s, x, y, WN_FG(wn), WN_BG(wn))

#define wn_tputs(wn,s,x,y)	wn_ttext(wn, s, x, y, WN_FG(wn), WN_BG(wn))
...
typedef void (*wn_close_func_t)wn__PROTO((int wn));
typedef int (*wn_get_cur_posn_func_t)wn__PROTO((void));
typedef void (*wn_overlay_insert_string_func_t)wn__PROTO((const char *text,
							  int pos,
							  int len,
							  int prepend_newline,
							  int fg_color,
...
typedef void (*wn_clipboard_changed_func_t)wn__PROTO((void*));
typedef void (*wn_iconify_notify_funct)wn__PROTO((int wn, int iconisize));
...
/* colors */
int wn_use_mono wn__PROTO((void));
void wn_use_color_hint wn__PROTO((int use_color));
void wn_npixels_hint wn__PROTO((int npixels));
int wn_get_pixels_by_color wn__PROTO((int wn, color_t *colors, int ncolors));
int wn_get_pixels wn__PROTO((int wn, color_t *colors, int ncolors));
int wn_get_pixels_and_planes wn__PROTO((int wn, int npixels, int nplanes, int contig, unsigned long *pixels, unsigned long *p_planes));
void wn_set_pixel_colors wn__PROTO((int wn, color_t *colors, int ncolors));
void wn_get_pixel_colors wn__PROTO((int wn, color_t *colors, int ncolors));
void wn_free_pixels wn__PROTO((int wn, color_t *colors, int ncolors));
int wn_parse_color wn__PROTO((int wn, const char *name, color_t *color));
...
void wn_xtext wn__PROTO((int wn, font_t *font, const char *s, int x, int y, int width,  int ropfunc, int fg_color, int bg_color, int use_which, int mono));
void wn_xwrite wn__PROTO((int wn, font_t *font, const char *s, int len, int x, int y, int ropfunc, int fg_color, int bg_color, int use_which, int mono));
...
void wn_set_cur_posn_func wn__PROTO((wn_get_cur_posn_func_t func));
void wn_set_overlay_insert_string_func wn__PROTO((wn_overlay_insert_string_func_t func));
void wn_overlay_insert_string wn__PROTO((const char *text, int pos,
...
void wn_suggest_resize_inc wn__PROTO((int base_width, int base_height,
						int width_inc, int height_inc));
void wn_set_classname wn__PROTO((const char *name));
const char *wn_open_display wn__PROTO((void));
void wn_set_close_func wn__PROTO((wn_close_func_t func));
void wn_set_icon_notify_func wn__PROTO((wn_iconify_notify_funct func));
int wn_open_stdwin wn__PROTO((void));
int wn_open_scr_stdwin wn__PROTO((const char* name, int xscreen));
int wn_create_window wn__PROTO((const char *name, int xscreen));
...
void wn_set_clip wn__PROTO(( int wn, short x, short y, short width, short height ));
void wn_clear_clip wn__PROTO((int wn));
...
void wn_set_default wn__PROTO((const char *name, const char* value));
...
int wn_multi_screen wn__PROTO((void));
int wn_intersects_rect wn__PROTO(( int wn, int x, int y, int width, int height ));
...
/* selection handling */
void wn_set_selection wn__PROTO((int wn, const char *buf, int nbytes));
void wn_get_selection wn__PROTO((int wn, const char **p_buf, int *p_nbytes));

  /* Use the following two functions instead to arrange that the selection
  ** stays highlighted until another selection is made.
  **   funct_parm is called when the selection changes (RCB).
  */
void wn_set_xclipboard wn__PROTO((int wn, const char* buf, int nbytes,
				long timestamp, wn_clipboard_changed_func_t,
				void* funct_parm));
void wn_clr_xclipboard wn__PROTO((void));


/* getting window attributes */
void wn_get_window_size wn__PROTO((int wn, int *p_width, int *p_height));
void wn_get_window_rect wn__PROTO((int wn, int *p_xpos, int *p_ypos, int *p_width, int *p_height));
int wn_get_root_window wn__PROTO((int wn));
int wn_same_root_window wn__PROTO((int wn1, int wn2));
...
int wn_get_nplanes wn__PROTO((int wn));
int wn_use_alloc_color wn__PROTO((int set));
int wn_use_alloc_color_for_highlight wn__PROTO((int set));
int wn_use_menu_colors wn__PROTO((void));
int wn_use_extended_text_colors wn__PROTO((void));
...
void wn_set_window_hints wn__PROTO((int wn, const char* title, const char* icontitle, bitmap_t* icon));

void wn_raise_window wn__PROTO((int wn));
void wn_map_window wn__PROTO((int wn));
void wn_unmap_window wn__PROTO((int wn));
void wn_iconify_window wn__PROTO((int wn));
void wn_lower_window wn__PROTO((int wn));
...
/* input */
long wn_set_input_timeout wn__PROTO((long msec));
...
int wn_open_rootwindow wn__PROTO((int xscreen));
...
void set_window_error_handler wn__PROTO((void));
int wn_expose_or_press_queued wn__PROTO((int wn, int md, int *p_x, int *p_y,
					 int *exposed, int *resized,
					 int *e_x, int *e_y, int *e_w, int *e_h));
int wn_text_capture wn__PROTO((int set, int reset, int *y));
void wn_do_flush wn__PROTO((void));
#endif /* !WN_H_INCLUDED */
