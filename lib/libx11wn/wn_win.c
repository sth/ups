...
/* @(#)wn_win.c	1.23 1/3/93 (UKC) */
char wn_wn_win_c_rcsid[] = "$Id$";
...
swin_t* w;
{
    static int root_table_size;
    if ( root_table_size == 0)
    {
	root_table_size = 1;
	wn_RootWintab = (swin_t**)wn__e_malloc(root_table_size * sizeof(swin_t*));
    }
    else if ( wn_NRootwin == root_table_size)
    {
	root_table_size *= 2;
	wn_RootWintab = (swin_t**)realloc((char *)wn_RootWintab,
					(size_t)root_table_size * sizeof(swin_t*));
    }
    if (wn_RootWintab == NULL)
		wn__panic("malloc/realloc failed in wn_record_root");

    wn_RootWintab[wn_NRootwin++] = w;
}
 ...
 *  If 'xscreen' is -1 open window on the default screen.
 */
static swin_t *
make_w(wn, name, xscreen)
int wn;
const char *name;
int xscreen;
{
	...
	w->w_bw->bw_screen = NULL;
#ifdef X11
	w->w_bw->bw_is_mapped = TRUE;
	if ((xscreen >= 0) && (xscreen <= ScreenCount(wn__Dpy)))
		w->w_bw->bw_screen = _wn_get_screen (xscreen);
	else
		w->w_bw->bw_screen = _wn_get_screen (DefaultScreen(wn__Dpy));
	_wn_set_x_gc(w->w_bw);
#endif
	w->w_bw->bw_buttons = 0;
...
#endif /* X11 */
	_wn_Windowtab[wn] = w;
	if (_Xdebug) printf("ups: [%2d] wn_install_wid(%ld)\n", wn, wid);
	return wn;
}
...
int xscreen;
{
	...
	_wn_Windowtab[wn] = w;
	if (_Xdebug) printf("ups: [%2d] wn_create_window(%s,%d)\n", wn, WN_PNULL(name), xscreen);
	...
	if (Want_own_icons) {
		...
		iw = make_w(iwn, name, xscreen);
		if (_wn_create_icon_win(iw, w, name) == -1) {
			wn_close_window(wn);
			return -1;
		}
		_wn_Windowtab[iwn] = iw;
		if (_Xdebug) printf("ups: [%2d] wn_create_window(%s,%d) icons\n", iwn, WN_PNULL(name), xscreen);
		iw->w_bw->bw_assocw = w;
...
#endif /* X11 */
#ifdef SUNVIEW
	wn_set_area(wn, 0, 0, w->w_width, w->w_height, W_BG_PIXEL(w));
#endif /* SUNVIEW */
	first_call = FALSE;
	return wn;
}
...
int xscreen;
{
	...
	if ((rwn = find_wn(WN_ANY)) == -1)
		return -1;
	rw = make_w(rwn, "Root window", xscreen);
	rw->w_win = RootWindow(wn__Dpy, W_SCREEN(rw));
	wn__update_size(rw);
	_wn_Windowtab[rwn] = rw;
	if (_Xdebug) printf("ups: [%2d] wn_open_root_window(%d)\n", rwn, xscreen);
	return rwn;
#endif /* X11 */
}
...
void
wn_get_window_rect(wn,p_xpos,p_ypos,p_width,p_height)
int wn;
int *p_xpos, *p_ypos;
...
/* Get the root window containing a given window */
int
wn_get_root_window(wn)
int wn;
{
	swin_t *w = WN_TO_W(wn);
	while (w->w_parent)
...
bool
wn_same_root_window(wn1, wn2)
int wn1;
int wn2;
{
	swin_t *w1 = WN_TO_W(wn1);
	swin_t *w2 = WN_TO_W(wn2);
	return w1->w_bw == w2->w_bw;
}
...
swin_t *par;
{
	...
	if (_Xdebug) printf("ups: [%2d] deleted.\n", par->w_wn);
...
void
wn_set_clip( int wn, short x, short y, short width, short height )
{
	swin_t *w;
	XRectangle rect;
	...
	if ( w->w_clip )
	{
		XDestroyRegion( w->w_clip );
		w->w_clip = NULL;
	}
	if ( w->w_clip == NULL ) w->w_clip = XCreateRegion();
	...
	XUnionRectWithRegion( &rect, w->w_clip, w->w_clip );
	XSetRegion( wn__Dpy, W_GC(w), w->w_clip );
}


void
wn_clear_clip( int wn )
{
	swin_t *w;
	XRectangle rect;
	...
	rect.x = 0;
	rect.y = 0;
	rect.width = w->w_width;
	rect.height = w->w_height;
	XUnionRectWithRegion( &rect, w->w_clip, w->w_clip );
	XSetRegion( wn__Dpy, W_GC(w), w->w_clip );
	XFlush( wn__Dpy );
}
...
int
wn_intersects_rect( int wn, int x, int y, int width, int height )
{
	swin_t *w;
	int l_xpos, l_ypos, l_width, l_height;
	int is_x, is_x2, is_y, is_y2;
	...
	is_x = MAX( l_xpos, x );
	is_x2 = MIN( l_xpos + l_width, x + width );
	if ( is_x < is_x2 )
	{
		/* possible */
		is_y = MAX( l_ypos, y );
		is_y2 = MIN( l_ypos + l_height, y + height );
		if ( is_y < is_y2 )
		{
			return TRUE;
		}
