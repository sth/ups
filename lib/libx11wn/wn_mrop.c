...
/* @(#)wn_mrop.c	1.13 1/3/93 (UKC) */
char wn_wn_mrop_c_rcsid[] = "$Id$";
...
#ifdef X11
#define N_PLANES(w)	DisplayPlanes(wn__Dpy, W_SCREEN(w))
#endif /* X11 */
#ifdef SUNVIEW
...
int fg, bg;
{
...
#ifdef X11
	_wn_send_bm(bm, sx, sy, width, height, w->w_win, W_GC(w), dx, dy,
						ropfunc, PIX(w,fg), PIX(w,bg));
	X_UPDATE(w);
#endif /* X11 */
	...
	if (bg != W_BG_PIXEL(w) && bm->bm_nplanes == 1) {
...
int dx, dy, ropfunc;
{
	...
	tbm = wn_make_bitmap_from_data(width, height,
			     DisplayPlanes(wn__Dpy, W_SCREEN(w)),
...
int x,y,width,height;
{
	...
	XSetState(wn__Dpy, W_GC(w), W_FG_PIXEL(w), W_BG_PIXEL(w),
				      GXcopy, AllPlanes);
	XSetGraphicsExposures(wn__Dpy, W_GC(w), FALSE);
	XCopyArea(wn__Dpy, w->w_win, sa->sa_pixmap, W_GC(w), x, y,
...
#else
	sa->sa_bm = wn_make_bitmap(width,height,wn_get_nplanes(wn),
...
long lsa;
{
	...
	if (sa->sa_pixmap != None) {
		XSetState(wn__Dpy, W_GC(w), W_FG_PIXEL(w), W_BG_PIXEL(w),
					   GXcopy, AllPlanes);
		XSetGraphicsExposures(wn__Dpy, W_GC(w), FALSE);
		XCopyArea(wn__Dpy, sa->sa_pixmap, w->w_win, W_GC(w), 0, 0,
	...
	if (sa->sa_bm != NULL)
		wn_put_image(sa->sa_bm,
			     ...
			     sa->sa_wn,
			     sa->sa_x, sa->sa_y,
			     R_RPL, W_FG_PIXEL(w), W_BG_PIXEL(w));
#endif /* !SUNVIEW */
#endif /* !X11 */
}
