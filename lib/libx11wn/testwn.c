...
/* @(#)testwn.c	1.25 31/7/94 (UKC) */
char wn_testwn_c_rcsid[] = "$Id$";
...
static int pixnum PROTO((int wn, int n));
static void init_colors PROTO((int wn));
...
static int async_mode PROTO((int group, int arg, int was_set));
static void flag_abort PROTO((void));
static void dump_key_state PROTO((int ch, int buttons));
static void dump_modifiers PROTO((int buttons));
...
/* Dummy Mgetmd() to allow program to build. */
int
Mgetmd(x,y,wn)
int x, y;
int wn;
...
int wn;
{
	...
	if (getenv("TESTCOLOR") != NULL) {
		if (wn_get_pixels_by_color(wn, Colors, NCOLORS) != 0) {
...
int buttons;
{
	if (buttons & B_SHIFT_MASK)
		fputs("shift-", stdout);
	if (buttons & B_CONTROL_MASK)
		fputs("control-", stdout);
	if (buttons & B_ALT_MASK)
		fputs("alt-", stdout);
}

static void
dump_key_state(ch, buttons)
int ch;
int buttons;
{
	int fkey;
	...
	fkey = ch - (unsigned char)WN_CH_FUNCKEY(0);
...
event_t *ev;
{
	...
	switch(ev->ev_type) {
	case EV_KEY:
		printf("key %d ", (unsigned char)ev->ev_char);
		dump_key_state((unsigned char)ev->ev_char, ev->ev_buttons);
		break;
	case EV_BUTTON_DOWN:
	...
	case EV_OTHER:
		...
		break;
	default:
		printf("UNKNOWN 0%lo", ev->ev_type);
	}
	putchar(']');
	fflush(stdout);
}
...
static int
async_mode(group, arg, was_set)
int group, arg, was_set;
{
	wn_abort_func_t oldfunc;
	char c;

	fputs("Hit newline to set abort trap: ", stdout);
	fflush(stdout);
	getchar();
	
	oldfunc = wn_set_abort_func(WN_STDWIN, flag_abort);
	...
	wn_set_abort_func(WN_STDWIN, oldfunc);
...
int wn, x, y, x_offs, y_offs, on;
{
	wn_set_area(wn, x + x_offs * BOX_W/2, y + y_offs * BOX_H/2,
		    BOX_W/2, BOX_H/2, on ? Colors[GREEN].co_pixel : WN_BG(wn));
}
...
int wn, x, y;
{
	...
	width = new_x - x;
	height = new_y - y;
	bm = wn_make_bitmap(width, height, wn_get_nplanes(wn), 
...
int wn;
{
	...
	for (i = 0, y = IMARG; i < N_SLOTS; i++, y += slot_h) {
		...
		wn_box_round(mwn, x, y, w, h, WN_FG(wn));
		sprintf(buf, "%d", i);
...
int nwins;
{
	...
	for (;;) {
		...
		if (event.ev_type & (EV_WINDOW_EXPOSED | EV_WINDOW_RESIZED)) {
			...
			color[i] = (color[i] == WN_FG(event.ev_wn)) ? WN_BG(event.ev_wn) : WN_FG(event.ev_wn);
			continue;
		}
...
#define NPIXELS 2
	...
	unsigned long pixels[NPIXELS], planes;
	font_t *font;

	if (wn_get_pixels_and_planes(wn, NPIXELS, 1, TRUE, pixels, &planes) != 0) {
		fprintf(stderr, "wn_get_pixels and planes failed");
		exit(1);
	}
	
	colors[0].co_pixel = WN_BG(wn);
	wn_get_pixel_colors(wn, &colors[0], 1);
	colors[2] = colors[0];
	...
	wn_set_pixel_colors(wn, colors, 4);
...
int which;
{
	...
	if (want_textbox) {
		for (i = 0; i <= len; ++i) {
			...
			wn_draw_line(wn, xpos, y, xpos, y + font->ft_height, WN_FG(wn));
		}
	}

	base_y = y + font->ft_baseline;

	wn_draw_line(wn, x - 8, base_y, x - 4, base_y, WN_FG(wn));
	wn_draw_line(wn, x + wn_strwidth(s, font) + 4, base_y,
			 x + wn_strwidth(s, font) + 8, base_y, WN_FG(wn));
	...
	for (i = 0; i < len; i += 2) {
		...
		wn_draw_line(wn, x1, base_y, x2, base_y, WN_FG(wn));
	}
}
...
const char **argv;
{
	...
	if (argc > 1 && strcmp(argv[1], "-inc") == 0) {
		if (argc < 5) {
			fprintf(stderr, "Usage: -inc base_width base_height width_inc height_inc\n");
			exit(1);
		}

		wn_suggest_resize_inc(atoi(argv[2]), atoi(argv[3]),
	...
	if (getenv("TESTPICS") != NULL) {
		...
		for (i = 0; i < N_IMAGES; i++) {
			...
			wn_put_image(bm,
				     ...
				     10, ly,
				     R_RPL,
				     pixnum(wn, Images[i].i_fg),
				     pixnum(wn, Images[i].i_bg));
	...
	if (argc > 2 && strcmp(argv[1], "-nwins") == 0) {
		...
		for (i = 1; i < nwins; ++i) {
			...
			if ((wntab[i] = wn_create_window(wname, -1)) == -1) {
...
redraw:
	...
	wn_updating_off(wn);
	if (wn_get_nplanes(wn) == 1 || getenv("TESTCOLOR") == NULL)
		wn_set_area(wn,0,0,w,h,WN_BG(wn));
	else {
		wn_set_area(wn, 0, 0, w, h/3, Colors[FGREEN].co_pixel);
		wn_set_area(wn, 0, h/3, w, h/3, Colors[KHAKI].co_pixel);
		wn_set_area(wn, 0, (h+h)/3, w, h/3, WN_BG(wn));
