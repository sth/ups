...
/* @(#)wn_misc.c	1.32 25 May 1995 (UKC) */
char wn_wn_misc_c_rcsid[] = "$Id$";
...
size_t nbytes;
{
	...
	ptr = malloc(nbytes);
	memset(ptr, 0, nbytes);	/* RGA for purify */
	if (ptr == NULL)
...
static wn_clipboard_changed_func_t clipboard_changed_funct;
static void* clipboard_changed_parm;

#ifdef SUNVIEW
static char *get_sel_data;
bool wn__Lost_selection = FALSE;
#endif /* SUNVIEW */

void
wn_set_selection(wn, buf, nbytes)
int wn;
const char *buf;
int nbytes;
{
    wn_set_xclipboard(wn,buf, nbytes, CurrentTime,
       (wn_clipboard_changed_func_t)NULL, NULL);
}

void
wn_get_selection(wn, p_buf, p_nbytes)
int wn;
const char **p_buf;
int *p_nbytes;
{
	...
	old_inmode = wn_inmode(wn, _WN_MININPUT);
	...
	do {
		...
		do {
			if (XPending(wn__Dpy) > 0) {
				...
				FD_ZERO(&mask);
				FD_SET(ConnectionNumber(wn__Dpy), &mask);

				result = select(32, &mask, (fd_set *)NULL,
						     (fd_set *)NULL, &timeout);
...
void
wn_set_xclipboard(wn,buf, nbytes, timestamp, changed_func, funct_parm)
int wn;
const char* buf;
int nbytes;
long timestamp;
wn_clipboard_changed_func_t changed_func;
void* funct_parm;
{
   wn_clr_xclipboard();
   {
	...
	/*   This follows X11 program samples, but didn't work: */
	/* XSetSelectionOwner(wn__Dpy, XA_PRIMARY, w->w_win, timestamp); */
	XSetSelectionOwner(wn__Dpy, XA_PRIMARY, w->w_win, CurrentTime);
   ...
   clipboard_changed_funct = changed_func;
   clipboard_changed_parm = funct_parm;
}

void
wn_clr_xclipboard()
{
   if ( clipboard_changed_funct )
	(*clipboard_changed_funct)(clipboard_changed_parm);
   clipboard_changed_funct = NULL;
}

/* If the window is iconified, de-iconify it.
** Then raise the window to the top of the stacking order.
*/
void
wn_raise_window(wn)
int wn;
{
	...
	w = WN_TO_W(wn);
	XMapRaised(wn__Dpy, w->w_win);
	XFlush(wn__Dpy);
#endif /* X11 */
}

/* If the window is unmapped map it.
*/
void
wn_map_window(wn)
int wn;
...
/* Unmap a window.  This removes it from the display
** without creating an icon.
*/
void
wn_unmap_window(wn)
int wn;
...
int wn;
{
	...
	wn = wn_get_root_window(wn);
	w = WN_TO_W(wn);
	XIconifyWindow(wn__Dpy, w->w_win, W_SCREEN(w));
	XFlush(wn__Dpy);
#endif /* X11 */
}
...
int nbytes;
{
	...
	memcpy(copy, data, (size_t)nbytes);
	copy[nbytes] = '\0';
	return copy;
}
