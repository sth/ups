...
/* @(#)o_disp.c	1.13 26/4/92 (UKC) */
char obj_disp_rcsid[] = "$Id$";
...
static void dr_objline PROTO((struct lptrst *lptr, int scr_x,
			      int scr_y, bool all_lines));

static void dr_obj_internal PROTO((struct objst *obj, int scr_x,
				   int scr_y, bool all_lines));
...
int npixels;
{
	...
	for (lptr = start; lptr != lim; lptr = lptr->lp_next) {
		if (lptr == xover)
			drawing = !drawing;
		if (drawing)
			dr_objline(lptr, -Offs_x, lptr->lp_ypos-new_offs_y, FALSE);
	}
#ifdef DISPLAY_PART
	...
	if (lim != NULL && lim->lp_ypos < Offs_y + wndepth)
		dr_objline(lim, -Offs_x, lim->lp_ypos-new_offs_y, FALSE);
#endif /* DISPLAY_PART */
 ...
 */
void
set_obj_wn_all_lines(wn, all_lines)
window_t wn;
int all_lines;
{
	...
	wn_get_window_size(wn, &window_width, &window_height);
				...
				/* RGA adjust to multiple of char width */
	window_width = (int)(window_width/wn_get_sysfont()->ft_width) *
	  wn_get_sysfont()->ft_width;

	if (obj__Change_callback != NULL)
		(*obj__Change_callback)(obj__Change_callback_data,
					OBJ_NEW_WINDOW_HEIGHT, window_height);

	if ( last_width == -1 )	/* AMC enhancements */
	{
		/* First time, set up the resizable formats */
		resize_all_formats(window_width);
		set_all_child_indents();
	}
	if (last_width != -1 && Updating_state == OBJ_UPDATING_ON) {
		if (window_width != last_width)
		{
			resize_all_formats(window_width); /* AMC enhancements */
			do_lptrs(TRUE);
		}
		display_all_lines(Offs_x, Offs_y, all_lines);
	}
...
{
  display_all_lines(obj_x, obj_y, 0);
}

/*  Display the tree starting at obj_x, obj_y pixels offset
 */
void
display_all_lines(obj_x, obj_y, all_lines)
register int obj_x, obj_y;
int all_lines;
{
	...
	if (next == NULL)
		obj_y = ypos;
/* RGA use this so overlay capture gets blank lines for double height */
/* target object  */
/*#ifdef DISPLAY_PART*/
	if (lptr->lp_prev != NULL)
		dr_objline(lptr->lp_prev, -obj_x,
			   lptr->lp_prev->lp_ypos-obj_y, all_lines);
/*#endif*/ /* DISPLAY_PART */
	if (ypos > obj_y)
	 ...
	 */
	while (lptr != NULL && (all_lines ||
				ypos + lptr->lp_ldepth - obj_y < win_height)) {
		wn_set_area(Obj_wn, 0, ypos - obj_y,
			    win_width, lptr->lp_ldepth, _ob_Default_bg_pixel);

		dr_objline(lptr, -obj_x, ypos - obj_y, all_lines);
		ypos += lptr->lp_ldepth;
		lptr = lptr->lp_next;
	}
 ...
 */
static void
dr_objline(lptr, scr_x, scr_y, all_lines)
struct lptrst *lptr;
int scr_x, scr_y;
bool all_lines;
{
	...
	for(;;) {
		...
		dr_obj_internal(obj, scr_x + xpos, scr_y, all_lines);
		xpos += obj->ob_width;
...
int scr_x, scr_y;
{
  dr_obj_internal(obj, scr_x, scr_y, FALSE);
}

static
void
dr_obj_internal(obj, scr_x, scr_y, all_lines)
register struct objst *obj;
int scr_x, scr_y;
bool all_lines;
{
	...
	if (Odesc[obj->ob_type].od_get_color != NULL)
		(*Odesc[obj->ob_type].od_get_color)(obj->ob_code, fdets.dr_wn,
								  &fdets.dr_fg,
								  &fdets.dr_bg);
	else {
	...
	for (flnptr = Odesc[obj->ob_type].od_fldln; flnptr != NULL;
		...
		for (fdescp = flnptr->fl_fdesc; fdescp != NULL;
			...
			if (all_lines ||
			    (fdets.dr_y>=0 && fdets.dr_y+fdets.dr_depth
					      < objwn_depth)) {
				...
				if (wn_use_alloc_color_for_highlight(0))
				{
				  ...
				  wn_set_area(fdets.dr_wn,
					      fdets.dr_x, fdets.dr_y,
					      obj->ob_width, fdets.dr_depth,
					      fdets.selected ?
	 ...
	 */
	if (obj->ob_flags & OB_HIGHLIGHTED && scr_y >= 0) {
	  if (!wn_use_alloc_color_for_highlight(0))
