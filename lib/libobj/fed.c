...
/* @(#)fed.c	1.25 09 Sep 1994 (UKC) */
char shared_fed_c_rcsid[] = "$Id$";
...
static void get_act PROTO((edesc_t *edesc));
static void preprocess_key PROTO((edesc_t *edesc));
static void preprocess_puck PROTO((edesc_t *edesc));
static int cancel_edit PROTO((edesc_t *edesc));
static void paste_selection PROTO((edesc_t *edesc));
static void redraw_string PROTO((edesc_t *edesc));
static char *make_copy PROTO((const char *s, size_t len));
...
/*  Initialise edesc_t. Set ed_{wn,orig,maxlen,scrlen}
...
int maxlen, fg, bg;
{
	...
	edesc->ed_copy = make_copy(orig, (size_t)maxlen);
...
edesc_t *edesc;
{
	...
	x = edesc->ed_puck_x;
	y = edesc->ed_puck_y;
	buttons = edesc->ed_buttons & (B_ANY | B_SHIFT_MASK);
	...
	else if (buttons == (B_MIDDLE | B_SHIFT_MASK)) {
...
int meaning;
{
	...
	case EDM_ILLEGAL_CHAR:
		ch = edesc->ed_char;
		if ((unsigned char)ch == ch  && isprint(ch))
...
edesc_t *edesc;
{
	...
	wn_get_selection(edesc->ed_wn, &buf, &nbytes);
...
const char *s;
{
	...
	if (edesc->ed_copy != NULL)
		free(edesc->ed_copy);
	edesc->ed_copy = make_copy(s, (size_t)edesc->ed_maxlen);
...
edesc_t *edesc;
{
	...
	wn_set_area(wn, 0, 0, wn_get_width(wn), wn_get_height(wn), WN_BG(wn));
...
edesc_t *edesc;
{
	...
	wn_set_area(wn, 0, 0, oldwidth, height, WN_BG(wn));
