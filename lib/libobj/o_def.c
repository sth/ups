...
/* @(#)o_def.c	1.14 09 Apr 1995 (UKC) */
char obj_def_rcsid[] = "$Id$";
...
static char *default_format PROTO((objid_t code));
 ...
 *  the syntax of format is (includes AMC enhancements)
 *
 *	format = ['\n'] format1
 ...
 *	varspec = '%' ['*'] ['[' userinfo ']'] [elastic_flag][size]['.' size] typechar
 *	elastic_flag = [min_size]':'
 *	min_size = ':'size
 *	size =  number  ['c']
 *      elastic = '<' or '<='
...
obj_get_color_func_t getcolor;
{
	...
	while (*format != '\0' && *format != '\n') {
		if (*format == '%' && format[1] != '%') {
			...
			/*    (AMC enhancements)
			**    See if this field is resizes
			*/
			if ( *format == ':')
			{
			    const char* e_start; /* remember this point */
			    e_start = (char *)++format;
			    resizes = TRUE;
			    flags |= OB_RESIZES;
			    min_width = uatoi(&format, DEF_WIDTH * Xscale, Xscale);
			    if ( *format == ':' )
				format++;
			    else
			    {
				/*   Got max size only.  Back up and parse
				**   it again
				*/
				min_width = 0;
				format = e_start;
...
/* RGA make width an exact multiple of font width so display */
/* overlay matches */
			if (width%Xscale)
			  width = (int)(width/Xscale)*Xscale;
			if (*format == '.')
				format++;
		...
		if ( resizes )  /* AMC enhancements */
		    fdesc->fd_resizes = width;
		else
		    fdesc->fd_resizes = 0;
		fdesc->fd_min_width = min_width;
		resizes = FALSE;
		min_width = 0;
	...
	p_odesc->od_format_obj = default_format;
...
void
resize_format(p_odesc, width)	/* AMC enhancements */
struct odescst *p_odesc;
int width;
{
	struct fldlnst *fldln;
	struct fdescst *elastic_fdesc, *fdesc;
	int obj_width = 0;
	int total_width, num_elastic;
	int new_width;
	font_t *font;

	if (!(p_odesc->od_flags & OB_RESIZES ))
	       return;  /* Nothing to do */

	font = wn_get_sysfont();
	for ( fldln = p_odesc->od_fldln; fldln; fldln=fldln->fl_next)
	{
	    elastic_fdesc = NULL;
	    total_width = p_odesc->od_child_indent;
	    for (num_elastic = 0, fdesc = fldln->fl_fdesc; fdesc; fdesc=fdesc->fd_next)
	    {
		if ( fdesc->fd_resizes)
		{
		    num_elastic++;
		} else
		    total_width += fdesc->fd_width;
	    }

	    new_width = font->ft_width*
	      (int)((width - total_width)/num_elastic/font->ft_width);
	    for (fdesc = fldln->fl_fdesc; fdesc; fdesc=fdesc->fd_next)
	    {
	      if ( fdesc->fd_resizes)
	      {
		elastic_fdesc = fdesc;
		if ( new_width > elastic_fdesc->fd_resizes/num_elastic )
		{
		    elastic_fdesc->fd_width = elastic_fdesc->fd_resizes/num_elastic;
		} else
		{
		    elastic_fdesc->fd_width = new_width;

		    /*   Allow for at least five character */
		    /*   With less the appending of "..." in pathname_draw
		    **   gets messed up! */
		    if ( elastic_fdesc->fd_width < elastic_fdesc->fd_min_width)
			elastic_fdesc->fd_width = elastic_fdesc->fd_min_width;
		}
		total_width += elastic_fdesc->fd_width;
	      }
	    }
	    if ( total_width > obj_width )
		obj_width = total_width;
	}
	p_odesc->od_width = obj_width;
}

void
resize_all_formats(width)	/* AMC enhancements */
int width;
{
    int i;
    for ( i = 0; i < MAX_OBJTYPES; i++)
	resize_format(&Odesc[i], width);
}

void
set_all_child_indents()
{
/* RGA make od_child_indent an exact multiple of font width so display */
/* overlay matches. Start at -1 so root object gets updated as well */

    int i, indent;

    indent = 3*wn_get_sysfont()->ft_width;
    for ( i = -1; i < MAX_OBJTYPES; i++)
	Odesc[i].od_child_indent = indent;
}
...
void
set_objtype_format_func(type, func)
int type;
obj_format_func_t func;
{
	if (type < 0 || type >= MAX_OBJTYPES)
		panic("type out of range in sodf");
	
	Odesc[type].od_format_obj = func;
}
...
objid_t code;
{
	panic("no format function defined for this object type");
...
/*  RGA like s_draw, but does an elastic draw. Use in target, add env entry,
 *  env strings, globals and local variables. Conforms to tool resizing
 */
...
register struct drawst *dets;
{
	...
	if (dets->dr_fval != NULL)
	{
	  /* RGA use wn_wd_ttext() so field values rescale */
	  if (dets->selected)
	    wn_wd_ttext(dets->dr_wn, (char *)dets->dr_fval, x, dets->dr_y,
