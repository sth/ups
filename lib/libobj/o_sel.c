...
/* @(#)o_sel.c	1.18 21/10/94 (UKC) */
char obj_sel_rcsid[] = "$Id$";
...
#endif
#elif HAVE_SELECT
static struct timeval timeout = { 0L, 10L };
#else
error Neither poll nor select
#endif
...
static void update_size PROTO((struct objst *obj));
...
void
obj_update_size(objcode)
objid_t objcode;
{
	update_size(code_to_obj(objcode));
}
...
struct objst *
select_list(x_threshold, threshold_exceeded)
     int x_threshold;
     int *threshold_exceeded;
{
	int puck_x, puck_y, obj_x, obj_y, xpos, forwards, last_forwards;
	int orig_x, orig_y, orig_ldepth, below, button, old_inmode;
	int objwn_height, scrolled, flipped, old_puck_x, old_puck_y;
	...
	*threshold_exceeded = 0;
	...
	if (obj == NULL)
	{
	  wn_ungetpuck(Obj_wn, old_puck_x, old_puck_y, B_LEFT);
	  *threshold_exceeded = -1;
	  return(0);
	}
	orig_type = obj->ob_type;
	...
	while (wn_getpuck(Obj_wn, &puck_x, &puck_y) & button) {
	  if (old_puck_x - puck_x > x_threshold ||
	      old_puck_x - puck_x < -x_threshold)
	  {
	    *threshold_exceeded = 1;
	    wn_ungetpuck(Obj_wn, old_puck_x, old_puck_y, B_LEFT);
	    break;
		...
		{
		  /* RGA add auto scrolling at top and bottom */
/*		  if(!scrolled)*/
		  {
		    if (puck_y> objwn_height)
		    {
		      scrolled = 1;
		      v_scroll(lptr->lp_ldepth);
		      obj_y -= lptr->lp_ldepth;
		      orig_y += lptr->lp_ldepth;
#if HAVE_POLL
		      poll(&unused, 0, 10);
#else
		...
		do {
			...
			{
			  below = FALSE;
			  if (scrolled && last_forwards == forwards)
			    flipped = 1;
			}
			else if (obj_y >= orig_y + orig_ldepth)
			{
			  ...
			  if (scrolled && last_forwards == forwards)
				...
				below = obj_x >= orig_x;
			if (below!=forwards &&
			   (obj_x!=orig_x || obj_y!=orig_y))
			  sel_obj(obj, flipped?SEL_ON:SEL_FLIP, SEL_PUCKSELECT);
			if (forwards) {
				if (obj->ob_flags & OB_LAST_ON_LINE) {
					obj_y += lptr->lp_ldepth;
					lptr = lptr->lp_next;
	...
	return (orig_type == obj->ob_type ? obj : 0);
}
...
void
select_object_at(puck_x, puck_y)
int puck_x, puck_y;
{
	struct objst *obj;
	int junk;
