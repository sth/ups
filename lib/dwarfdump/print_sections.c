...
static Dwarf_Unsigned local_dwarf_decode_u_leb128(
    unsigned char        *leb128,
...
get_proc_name(Dwarf_Debug dbg, Dwarf_Die die, Dwarf_Addr low_pc,char *proc_name_buf,int proc_name_buf_len)
{
	...
	if(ares == DW_DLV_ERROR) {
	  ...
	  case DW_AT_name:
	    ...
	    if(sres == DW_DLV_ERROR) {
		print_error(dbg,"formstring in get_proc_name failed",sres,err);
		safe_strcpy(proc_name_buf,proc_name_buf_len,"ERROR in dwarf_formstring!",50/* safe wrong length since is big */);
	    } else if (sres == DW_DLV_NO_ENTRY) {
		safe_strcpy(proc_name_buf,proc_name_buf_len,"NO ENTRY on dwarf_formstring?!",50/* safe wrong length since is big */);
	    } else {
	  ...
	  case DW_AT_low_pc:
		...
		print_error(dbg,"formaddr in get_proc_name failed",dres,err);
		low_pc_die = ~low_pc;
		/* ensure no match */
	    } 
	    funcpcfound = 1;
	...
	ret_name_buf[0] = 0;
		  ...
		  int lchres;
		    ...
		    int proc_name_v;
				...
				name_buf,BUFSIZ);
                    if(proc_name_v) {
                        /* this is it*/
			safe_strcpy(ret_name_buf,ret_name_buf_len,name_buf,strlen(name_buf));
		  	if(die_locally_gotten) {
		      	/* If we got this die from the parent, we don not
		         	want to dealloc here!
		      	*/
                        	dwarf_dealloc(dbg, curdie, DW_DLA_DIE);
		      	}
			return 1;
		    }
		    /* check children of subprograms recursively
		       should this really be check children of
			anything?
		    */
			...
			int newprog = get_nested_proc_name(dbg,newchild,low_pc,name_buf,BUFSIZ);
			dwarf_dealloc(dbg,newchild,DW_DLA_DIE);
			if(newprog) {
				/* Found it.  We could just 
				   take this name or we could
				   concatenate names together
				   For now, just take name
				*/
			        safe_strcpy(ret_name_buf,ret_name_buf_len,name_buf,strlen(name_buf));
				return 1;
			}
			...
			/* nothing to do */
                         ...
                         print_error(dbg, "get_nested_proc_name dwarf_child() failed ", chres,err);                
		  	 if(die_locally_gotten) {
		      	 /* If we got this die from the parent, we don not
		         	want to dealloc here!
		      	 */
		...
		/* try next sibling */
                ...
                if(chres == DW_DLV_ERROR) {
                  print_error(dbg, "dwarf_cu_header On Child read ", chres,err);
		  if(die_locally_gotten) {
		  /* If we got this die from the parent, we don not
		     want to dealloc here!
		  */
...
get_fde_proc_name(Dwarf_Debug dbg, Dwarf_Addr low_pc)
{
	...
	chres = dwarf_child(current_cu_die_for_print_frames,&child, &err);
	if(chres == DW_DLV_ERROR) {
	    print_error(dbg, "dwarf_cu_header on child read ", chres,err);
	} else if (chres == DW_DLV_NO_ENTRY) {
	} else { /* DW_DLV_OK */
	  int gotname = 
		get_nested_proc_name(dbg, child, low_pc,proc_name,BUFSIZ);
	  if(gotname) {
	    	     return(proc_name);
	  }
	...
	current_cu_die_for_print_frames = 0;
	if(dres == DW_DLV_ERROR) {
	      print_error(dbg, "dwarf_cu_header Child Read finding proc name for .debug_frame", chres,err);
	      continue;
   ...
   for( ; len > 0; ) {
	...
	default:
	   loff = off;
		...
		default:
		   printf("Error: Unexpected address size %d in DW_CFA_set_loc!\n",addr_size);
		   uval = 0;
		}
	    ...
	    case DW_CFA_offset_extended:
                ...
                len -= uleblen;
                off += uleblen;
                uval2 = local_dwarf_decode_u_leb128(instp+1,&uleblen);
                instp += uleblen;
                len -= uleblen;
                off += uleblen;
        ...
        /* single uleb128 is the current arg area size
           in bytes.
           no register exists yet to save this in */
...
print_frames (Dwarf_Debug dbg)
{
	...
	if(framed == 0) {
	  ...
	  /* this is gnu g++ exceptions in a .eh_frame section.
	     Which is just like .debug_frame except that the
             empty, or 'special' CIE_id is 0, not -1
	     (to distinguish fde from cie).
	     And the augmentation is "eh".
	     As of egcs-1.1.2 anyway.
	     A non-zero  cie_id is in a fde and is the difference
	     between the fde address and the beginning of the
	     cie it belongs to. This makes sense as this is intended
	     to be referenced at run time, and is part of the
	     running image.
	  */
	  ...
	  silent_if_missing = 1;
	  framename = ".eh_frame";
	  fres = dwarf_get_fde_list_eh(dbg, &cie_data, &cie_element_count, 
				     &fde_data, &fde_element_count, &err);
	}
	...
	else if (fres == DW_DLV_NO_ENTRY)
	{
		...
		if (cu_name_flag && fde_offset_for_cu_low != DW_DLV_BADOFFSET
		...
		printf("<%3lld><%#llx:%#llx><%s><fde offset 0x%llx length: 0x%llx>", 
			cie_index, low_pc, 
		       (low_pc+func_length), temps ? temps : "",
			fde_offset,fde_bytes_length);
		...
		for (j = low_pc; j < low_pc+func_length; j++) {
			...
			if (fires  == DW_DLV_ERROR) {
			    print_error(dbg, "dwarf_get_fde_info_for_reg",fires, err);
			}
		...
		if(verbose > 1) {
			Dwarf_Off fde_off;
			Dwarf_Off cie_off;
			/* get the fde instructions and print them
			   in raw form, just like cie instructions */
			Dwarf_Ptr instrs;
			Dwarf_Unsigned ilen;
			int res;
			res = dwarf_get_fde_instr_bytes(fde_data[i],
				&instrs,&ilen,&err);
 			offres = _dwarf_fde_section_offset(dbg,fde_data[i],
                        	&fde_off,&cie_off,&err);
                        if(offres == DW_DLV_OK) {
			...
			if(res == DW_DLV_OK) {
				...
				(long long)cie_index,(long long)i);
			     ...
			     print_frame_inst_bytes(dbg,instrs,ilen,
					data_alignment_factor,code_alignment_factor,address_size);
			  }
			} else if(res == DW_DLV_NO_ENTRY){
			  printf("Impossible: no instr bytes for fde index %d?\n",(int)i);
			} else {
				/* DW_DLV_ERROR */
			  printf("Error: on gettinginstr bytes for fde index %d?\n",(int)i);
	    ...
	    */
		...
		if (verbose) {
		    ...
		    cires = _dwarf_cie_section_offset(dbg,cie_data[i],
			&cie_off,&err);
		    if(cires == DW_DLV_OK) {
		    ...
		    printf("\treturn_address_register\t\t%d\n", 
			   return_address_register_rule);
		    printf("\tbytes of initial instructions:\t%lld\n", 
...
print_pubnames (Dwarf_Debug dbg)
{
	...
	if (res == DW_DLV_ERROR) {
		...
		/* (err == 0 && count == DW_DLV_NOCOUNT) means 
		   there are no pubnames.  */
	}
	else {
	    for (i = 0; i < count; i++) {
		int nres;
		...
		if (check_pubname_attr) {
		    ...
		    if(ares == DW_DLV_ERROR) {
			print_error(dbg,"hassattr on DW_AT_external",ares,err);
		    }
...
print_abbrevs (Dwarf_Debug dbg)
{
	...
	while ((abres = dwarf_get_abbrev (dbg, offset, &ab,
		...
		if(acres == DW_DLV_ERROR) {
		  print_error(dbg, "dwarf_get_abbrev_children_flag",acres, err);
		}
		...
		if (dense)
			...
			printf("%s\n", child_name);
		/* Abbrev just contains the format of a die,
		 * which debug_info then points to with the real data.
		 * So here we just print the given format. */
		for (i = 0; i < attr_count; i++) {
			int aeres;
			aeres = dwarf_get_abbrev_entry (ab, i,&attr, &form, &off, &err);
...
print_aranges (Dwarf_Debug dbg)
{
	...
	if (ares == DW_DLV_ERROR) {
		...
		aires =dwarf_get_arange_info(arange_buf[i], 
		     &start, &length, &cu_die_offset, &err); 
		if (aires != DW_DLV_OK) {
			...
			if(dres != DW_DLV_OK) { 
				...
				if(fres == DW_DLV_OK) {
				  if (theform == DW_FORM_string 
				    ...
				    string temps;
				    int sres;
				    sres = dwarf_formstring(attrib,&temps, &err);
				    if(sres == DW_DLV_OK) {
				      string p = temps;
				      if (cu_name[0] != '/') {
					p = strrchr(temps, '/');
...
print_static_funcs(Dwarf_Debug dbg)
{
	...
	for (i = 0; i < count; i++) {
	    ...
	    int cures;
	    int dores;
	    fnres = dwarf_func_name_offsets (funcbuf[i],&name, &die_off, &cu_off, 
					    &err);
	    if (fnres != DW_DLV_OK) {
...
print_static_vars(Dwarf_Debug dbg)
{
	...
	for (i = 0; i < count; i++) {
	    ...
	    vnres = dwarf_var_name_offsets (varbuf[i],&name, &die_off, &cu_off, &err);
	    if (vnres != DW_DLV_OK) {
