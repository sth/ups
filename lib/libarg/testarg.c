...
/* @(#)testarg.c	1.10 17 Apr 1994 (UKC) */
char arg_testarg_rcsid[] = "$Id$";
...
static void do_shellcmd PROTO((const char *line));
...
const char *line;
{
	...
	if (arg_make_shell_line(line, &shell_line) == 0) {
		errf("\"%s\" -> \"%s\"", line, shell_line);
		free(shell_line);
	}
}
